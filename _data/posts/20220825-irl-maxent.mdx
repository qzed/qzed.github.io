...
bibliography: 20220825-irl-maxent.bib.json
---

import EqRef from '../../components/blog/EqRef.tsx'
import Note from '../../components/blog/Note.tsx'
...
In particular, it assumes that the transition dynamics, i.e., the probability distribution $p(s_{t+1} \mid s_t, a_t)$, are known.
This may not be the case in many real-world scenarios.
Further, it assumes a linear parametrization of the reward $R(s) = \omega^\intercal \phi(s)$.
...
Lastly, maximum entropy IRL is constrained to limited transition randomness, i.e., fairly deterministic transition dynamics $p(s_{t+1} \mid s_t, a_t)$, due to the reward bias introduced by our probability approximation.

The latter can be avoided via the maximum _causal_ entropy IRL approach, proposed subsequently by Ziebart [@ziebartModelingPurposefulAdaptive2010;@ziebartPrincipleMaximumCausal2013].
Multiple other extensions to this algorithm have been suggested, for example for nonlinear reward functions via neural networks [@wulfmeierMaximumEntropyDeep2016].

While the algorithm shown here is limited to somewhat smaller discrete state- and action-spaces, it can be extended to continuous state spaces, for example via Monte-Carlo sampling or via path integrals [see e.g., @aghasadeghiMaximumEntropyInverse2011;@kalakrishnanLearningObjectiveFunctions2013].
